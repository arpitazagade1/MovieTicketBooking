...
<td><input type="text" name="Trailer" placeholder="trailar url" class=""form-control"></td>
...
<td><%= rs.getString("trailer_url")%></td>
<td>
<form action="AdminMovie.jsp" method="post">
<input type="hidden" name="id" value="<%=rs.getInt("movie_id") %>">
...
<input type="text" name="Trailer" value="<%=rs.getString("trailer_url")%>">
...
<!-- This is one single insert form in Insert column -->
<tr>
<td colspan="6"></td> <!-- empty cells for first 6 columns -->
<td></td> <!-- empty Update column -->
<td>
  <form action="AdminMovie.jsp" method="post">
    <input type="number" name="id" placeholder="Movie Id" class="form-control mb-1">
    <input type="text" name="title1" placeholder="Movie Name" class="form-control mb-1">
    <input type="text" name="description1" placeholder="Description" class="form-control mb-1">
    <input type="text" name="poster_url1" placeholder="Poster URL" class="form-control mb-1">
    <input type="text" name="movie_language1" placeholder="Language" class="form-control mb-1">
    <input type="text" name="time1" placeholder="Time" class="form-control mb-1">
    <input type="text" name="Trailer" placeholder="Trainler url" class="form-control mb-1">
    <button class="btn btn-success w-100" type="submit" name="submit">Insert</button>
  </form>
</td>
	...
	try{
		...
		Connection con3=DriverManager.getConnection("jdbc:mysql://localhost:3306/demo","root","root");
		 Statement st=con3.createStatement();
	      st.executeUpdate("insert into movies (movie_id,title ,description,poster_url,movie_langauge,time,trailer_url) values ('"+Id+"','"+Title+"','"+Desc+"','"+poster+"','"+lang+"','"+ Time +"','"+ Time +"''"+ trailer +"')");
...
if(request.getParameter("delete")!=null){
    try{
        ...
        PreparedStatement ps1=con1.prepareStatement("delete from movies where movie_id=?");
        ps1.setInt(1,Id);
        ps1.executeUpdate();
        response.sendRedirect("AdminMovie.jsp");
    }catch(Exception e){
...
if(request.getParameter("edit")!=null){
    try{
        ...
        String lang=request.getParameter("movie_language");
        String Time=request.getParameter("time");

        Class.forName("com.mysql.cj.jdbc.Driver");
        Connection con2=DriverManager.getConnection("jdbc:mysql://localhost:3306/demo","root","root");
        PreparedStatement ps2=con2.prepareStatement("update movies set title=? , description=? ,poster_url=? ,movie_langauge=? ,time=? where movie_id=?");
        ps2.setString(1,Title1);
        ps2.setString(2,Desc);
        ps2.setString(3, poste);
        ps2.setString(4,lang);
